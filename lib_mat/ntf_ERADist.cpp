  ...
  array<char, 2U> &opt, const coder::array<double, 2U> &val_temp)
{
  ...
  switch (loop_ub) {
   case 0:
    ...
    switch (loop_ub) {
     ...
     case 6:
      if ((val[0] > 0.0) && (val[1] > 0.0) && (val[2] < val[3])) {
        ...
        Mean_data[0] = Mean;
        Var_size_idx_1 = 1;
        Var_data[0] = Mean * val[1] / (s2 * (s2 + 1.0));
      }
      break;
     ...
     case 14:
      if (val[0] > 0.0) {
        ...
        //  CDF = cdf(name,x,val);
        Mean_data[0] = val[0] * 1.2533141373155001;
        Var_size_idx_1 = 1;
        Var_data[0] = 0.42920367320510344 * (val[0] * val[0]);
      }
      break;
     ...
     case 15:
      ...
      if ((val[0] > 0.0) && (ntf_mod(val[0]) == 0.0)) {
        ...
        Std_data[0] = val[0] / 2.0;
        Std_data[1] = val[1] / 2.0;
        Std_data[2] = val[2] / 2.0;
        Std_data[3] = val[3] / 2.0;
        ntf_gamstat(Std_data, val, b_Var);
        Mean_size_idx_1 = 4;
        Var_size_idx_1 = 4;
        Mean_data[0] = val[0];
        Var_data[0] = b_Var[0];
    ...
    Std_size[0] = 1;
    Std_size[1] = Var_size_idx_1;
    if (0 <= Var_size_idx_1 - 1) {
      std::memcpy(&Std_data[0], &Var_data[0], Var_size_idx_1 * sizeof(double));
    }

    ntf_sqrt(Std_data, Std_size);
   ...
   case 1:
    ...
    switch (loop_ub) {
     ...
     case 1:
      ...
      Mean_data[0] = val[0] - 1.0;
      Std_size[0] = 1;
      Std_size[1] = 1;
      Std_data[0] = std::sqrt((val[0] - 1.0) * ((val[0] - 1.0) + 1.0));
      break;
     ...
     case 5:
      ...
      Mean = val[1] * val[1];
      this->Par[1] = val[0] * val[0] / Mean;
      this->Par[0] = val[0] / Mean;
     ...
     case 6:
      ...
      s2 = val[3] - val[0];
      Var = val[3] - val[2];
      this->Par[0] = (s2 * Mean / (val[1] * val[1]) - 1.0) * Mean / Var;
      this->Par[1] = this->Par[0] * s2 / Mean;
      this->Par[2] = val[2];
      this->Par[3] = val[3];

      //  Evaluate if distribution can be defined on the parameters
      Mean_data[0] = (this->Par[1] * val[2] + this->Par[0] * val[3]) /
     ...
     case 11:
      ...
      if (val[0] == val[2]) {
        ...
        ntf_c_fzero(fun, Mean, &xs, &s2, &Var);
        if (Var > 0.0) {
          this->Par[0] = xs;
          Mean = 1.0 - xs;
          ntf_gamma(&Mean);
          this->Par[1] = (val[0] - val[2]) * xs / (Mean - 1.0);
          this->Par[2] = val[2];
        }
      }
     ...
     case 16:
      //  compute parameters
      Mean = 3.4641016151377544 * val[1] / 2.0;
      this->Par[0] = val[0] - Mean;
      this->Par[1] = val[0] + Mean;
     ...
     case 19:
      ...
      s2 = val[1] / val[0];
      Mean = s2 * s2 + 1.0;
      this->Par[0] = std::log(val[0]) - std::log(std::sqrt(Mean));

      //  mean normal
      this->Par[1] = std::sqrt(std::log(Mean));
  ...
  this->mean.size[0] = 1;
  this->mean.size[1] = Mean_size_idx_1;
  if (0 <= Mean_size_idx_1 - 1) {
    std::memcpy(&this->mean.data[0], &Mean_data[0], Mean_size_idx_1 * sizeof
                (double));
  }

  this->std.size[0] = 1;
  this->std.size[1] = Std_size[1];
  loop_ub = Std_size[0] * Std_size[1];
  if (0 <= loop_ub - 1) {
    std::memcpy(&this->std.data[0], &Std_data[0], loop_ub * sizeof(double));
  }
}
  ...
  double, 1U> &CDF) const
{
  ...
  static const char b_cv7[6] = { 'p', 'a', 'r', 'e', 't', 'o' };
  ...
  switch (loop_ub) {
   ...
   case 4:
    ntf_d_cdf(x, 1.0 / this->Par[0], CDF);
    break;

   case 5:
    ntf_e_cdf(x, this->Par[1], 1.0 / this->Par[0], CDF);
    break;
   ...
   case 6:
    {
      ...
      ntf_f_cdf(b_x, this->Par[0], this->Par[1], CDF);
    }
    break;
   ...
   case 9:
    ntf_g_cdf(x, 1.0 / this->Par[1], this->Par[0] / this->Par[1], this->Par[0],
              CDF);
    break;
   ...
   case 11:
    ntf_g_cdf(x, this->Par[0], this->Par[1], this->Par[2], CDF);
    break;
   ...
   case 12:
    ...
    ntf_g_cdf(b_x, this->Par[0], this->Par[1], -this->Par[2], CDF);
    break;
