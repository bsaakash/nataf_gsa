	...
	Kos, char opts, coder::array<double, 1U>& Si)
{
	...
	for (nc = 0; nc < i; nc++) {
		if (combs[nc].f1.size(1) == 0) {
			...
			gsa_do_vectors(xi, combs[nc].f1, c, ia, ib_size);
			//if (c.size(1) == 0) {
			//	Si[nc] = 1.0;
			//}
			//else {
				...
				if (empty_non_axis_sizes || ((x.size(0) != 0) && (combs[nc].f1.size(1)
					!= 0))) {
				...
				for (int b_i = 0; b_i < nsamp; b_i++) {
					...
					for (k = 0; k < i1; k++) {
						...
						z1.set_size(id1.size(1));
						for (i2 = 0; i2 < loop_ub; i2++) {
							z1[i2] = models[iindx].mu[(static_cast<int>(id1[i2]) +
						...
						loop_ub = id1.size(1);
						c.set_size(1, id1.size(1));
						for (i2 = 0; i2 < loop_ub; i2++) {
							c[i2] = models[iindx].mu[(static_cast<int>(id1[i2]) + models[iindx]
								.mu.size(0) * k) - 1];
						}
							...
							for (result = 0; result < b_loop_ub; result++) {
								b_models[result + b_models.size(0) * i2] = models[iindx].Sigma
									[((static_cast<int>(id1[result]) + models[iindx].Sigma.size(0)
										* (static_cast<int>(id1[i2]) - 1)) + models[iindx].
										Sigma.size(0) * models[iindx].Sigma.size(1) * k) - 1];
							}
						}

						pdfval = gsa_mymvnpdf(xi, c, b_models);
						pik_tempi[b_i + pik_tempi.size(0) * k] = models[iindx].w[k] * pdfval;
					}

					loop_ub = pik_tempi.size(1);
					c.set_size(1, pik_tempi.size(1));
					for (i1 = 0; i1 < loop_ub; i1++) {
						c[i1] = pik_tempi[b_i + pik_tempi.size(0) * i1];
					}
					...
					loop_ub = pik_tempi.size(1);
					for (i1 = 0; i1 < loop_ub; i1++) {
						piki[b_i + piki.size(0) * i1] = pik_tempi[b_i + pik_tempi.size(0) *
							i1] / pdfval;
					}
				}
				...
				for (i1 = 0; i1 < loop_ub; i1++) {
					...
					for (i2 = 0; i2 < b_loop_ub; i2++) {
						xsec[i2 + xsec.size(0) * i1] = piki[i1 + piki.size(0) * i2] *
							xall[i1 + xall.size(0) * i2];
					}
				}

				gsa_sum(xsec, c);
				result = c.size(1);
				b_c = c.reshape(result);
				Vi[nc] = gsa_rt_roundd_snf(gsa_vvarstd(b_c, c.size(1)) * 1.0E+15) *
					1.0E-15;
				Si[nc] = Vi[nc] / V;
